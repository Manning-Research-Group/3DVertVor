...
inline bool removeFolder(const std::string& folderName) {
...
inline bool recursivelyDeleteFolder(std::string folderName) {
  if(!removeFolderContent(folderName)) return false;
  if(!removeFolder(folderName)) return false;
  return true;
}
